...
{
    let inputPort = ns.getPortHandle(Bus.portMap["NET IN"]);
    if (!inputPort.empty())
    {
...
{
    let outputPort = ns.getPortHandle(Bus.portMap["NET OUT"]);
    let packet = new Packet(packetName, "NET", destination, data);
