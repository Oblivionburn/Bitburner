...
import {colors} from "/Hax/Util.js";
...
{
	ns.scp("/Hax/Weaken.js", host, "home");
	ns.scp("/Hax/Grow.js", host, "home");
	ns.scp("/Hax/Hack.js", host, "home");
	ns.scp("/Hax/RunBatch.js", host, "home");
}
...
function PurchasingEnabled(ns)
{
	...
	if (!configs)
	{
		...
		IO.Write(ns, "configs", [{Name: "PurchasingEnabled", Value: true}]);
	}
	else
	{
...
function PurchaseServers(ns)
{
	...
	let serverCost = ns.cloud.getServerCost(2);
	...
	if (money >= serverCost)
	{
		...
		let serverLimit = ns.cloud.getServerLimit();
		if (purchasedNum < serverLimit)
		{
			let server_name = "PS-" + purchasedNum + "-v1";
			ns.cloud.purchaseServer(server_name, 2);
...
function UpgradeServers(ns)
{
	...
	if (servers != null &&
			servers.length > 0)
	{
		let serverRamLimit = ns.cloud.getRamLimit();
		let updated = false;
			...
			if (ns.serverExists(server.Name))
			{
				if (server.Purchased)
				{
							...
							money >= upgradeCost)
					{
						...
						ns.cloud.deleteServer(server.Name);
						ns.cloud.purchaseServer(new_serverName, nextRam);
